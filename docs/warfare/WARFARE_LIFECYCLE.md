...
- `src/features/warfare/warfare-hooks.js` captures the base Warfare & Supply `initialize` and `simulateWarfareYear` functions immediately after the core engine loads.
...
- `src/features/warfare/warfare-lifecycle.js` remains as a compatibility facade and delegates to the hook registry; it no longer wraps `simulateWarfareYear`.
...
- Historical warfare extensions are loaded centrally by `src/features/worldbuilding/worldbuilding-loader.js`; relic collections no longer bootstrap scripts as a side effect.
- `tests/warfare/warfare-legacy-wrapper-source.test.js` statically guards the cleaned module set against lifecycle monkey-patch regressions.
