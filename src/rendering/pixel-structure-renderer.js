...
const VERSION='4.1.0',clamp=(v,a,b)=>Math.max(a,Math.min(b,v)),px=n=>Math.round(n),xmlAttr=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&apos;'}[c]));
...
function renderSettlement(profile,options={}){
 const W=Math.max(320,Number(options.width)||720),H=Math.max(220,Number(options.height)||440),p=palette(profile),r=rng(`${profile.previewSeed}|scene`),cx=W/2,cy=H/2,architecture=profile.architectureStyle||Archetypes.architecture(profile.cultureStyle,profile.biomeStyle,profile.flags),ds=profile.districts||[],requestedFocus=options.focusDistrictId==null?null:String(options.focusDistrictId),focusDistrict=requestedFocus==null?null:ds.find(d=>String(d.id)===requestedFocus)||null;
 const zoom=focusDistrict?clamp(Number(options.zoom)||2.45,1.4,4):1,viewWidth=px(W/zoom),viewHeight=px(H/zoom),focusX=focusDistrict?px(clamp(Number(focusDistrict.x||.5)*W-viewWidth/2,0,W-viewWidth)):0,focusY=focusDistrict?px(clamp(Number(focusDistrict.y||.5)*H-viewHeight/2,0,H-viewHeight)):0,viewBox=focusDistrict?`${focusX} ${focusY} ${viewWidth} ${viewHeight}`:`0 0 ${W} ${H}`,focusAttributes=focusDistrict?` data-worldforge-focus-district="${xmlAttr(focusDistrict.id)}" data-worldforge-focus-zoom="${zoom}"`:'';
 const ariaLabel=`Pixel settlement preview of ${String(profile.name||'settlement').replace(/[&<>"']/g,'')}${focusDistrict?`, district ${String(focusDistrict.name||focusDistrict.kind||'district').replace(/[&<>"']/g,'')}`:''}`;
 let out=`<svg data-worldforge-pixel="v4.0" data-worldforge-renderer="v4.1" data-architecture-family="${architecture.id}"${focusAttributes} viewBox="${viewBox}" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges" role="img" aria-label="${ariaLabel}">`;
 ...
 for(const d of ds){const x=d.x*W,y=d.y*H;out+=line(cx,cy,x,y,p.road,Math.max(3,W*.007))}
 ...
 for(const d of ds){const x=d.x*W,y=d.y*H,scale=Math.max(2,Math.floor(W/260)),entries=(d.structures||[]).slice().sort((a,b)=>b.count-a.count),focused=Boolean(focusDistrict&&String(d.id)===String(focusDistrict.id));out+=`<g data-district="${xmlAttr(d.kind)}" data-district-id="${xmlAttr(d.id)}" tabindex="0" role="button" aria-pressed="${focused}" aria-label="Open ${xmlAttr(d.name||districtLabel(d.kind))} district view"${focused?' class="district-focused"':focusDistrict?' opacity=".42"':''}>`;if(focused)out+=`<circle cx="${px(x)}" cy="${px(y)}" r="${Math.max(24,Math.floor(W*.075))}" fill="none" stroke="${p.accent}" stroke-width="3" stroke-dasharray="7 5"/>`;const total=Math.min(9,Math.max(3,Math.round(Math.sqrt(entries.reduce((n,a)=>n+a.count,0)))));for(let i=0;i<total;i++){const e=entries[i%Math.max(1,entries.length)]||{type:'house'},ang=(i/total)*Math.PI*2+r()*.3,rad=12+Math.floor(i/3)*10+Math.floor(r()*8),gx=clamp(x+Math.cos(ang)*rad,8,W-60),gy=clamp(y+Math.sin(ang)*rad,8,H-45);out+=structureGlyph(e.type,gx,gy,scale,p,`${profile.previewSeed}|${d.id}|${i}`,d.condition??profile.condition,profile)}out+=`<text x="${px(x)}" y="${px(y-10)}" text-anchor="middle" font-family="monospace" font-size="${Math.max(8,Math.floor(W/78))}" fill="${p.dark}" opacity=".88">${districtLabel(d.kind)}</text></g>`}
