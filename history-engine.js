...
function viewAt(world,year){const h=initialize(world),target=clamp(Math.round(Number(year)||0),0,h.currentYear);if(target===h.currentYear)return currentView(world);const entry=cachedViewEntry(world,h);if(entry.years.has(target))return entry.years.get(target);const view=viewFromSnapshot(world,h,snapshotAt(world,target));entry.years.set(target,view);return view}
