...
const viewCache=new WeakMap(),metricsCache=new WeakMap();
...
function historyVersion(h){return h.currentYear+'|'+h.snapshots.length+'|'+h.events.length}
function cachedViewEntry(world,h){const version=historyVersion(h);let entry=viewCache.get(world);if(!entry||entry.version!==version){entry={version,present:null,years:new Map};viewCache.set(world,entry)}return entry}
function currentView(world){const h=initialize(world),entry=cachedViewEntry(world,h);if(entry.present)return entry.present;const s={year:h.currentYear,settlementPopulation:h.current.settlementPopulation,settlementWealth:h.current.settlementWealth,settlementProsperity:h.current.settlementProsperity,settlementKingdomIds:h.current.settlementKingdomIds,politicalRLE:rleEncode(h.current.political),realms:snapshotRealmRows(h),relations:h.current.relations,alliances:h.current.alliances,activeWars:h.current.activeWarIds.map(id=>{const w=h.wars[id];return{id:w.id,attackerId:w.attackerId,defenderId:w.defenderId,startYear:w.startYear,score:w.score}}),tradeVolumes:h.current.tradeVolumes,metrics:currentMetrics(h),eventIndex:h.events.length};entry.present=viewFromSnapshot(world,h,s);return entry.present}
function viewAt(world,year){const h=initialize(world),target=clamp(Math.round(Number(year)||0),0,h.currentYear);if(target===h.currentYear)return currentView(world);const entry=cachedViewEntry(world,h);if(entry.years.has(target)){const view=entry.years.get(target);entry.years.delete(target);entry.years.set(target,view);return view}const view=viewFromSnapshot(world,h,snapshotAt(world,target));entry.years.set(target,view);if(entry.years.size>8)entry.years.delete(entry.years.keys().next().value);return view}
function metricsSeries(world){const h=initialize(world),version=historyVersion(h),cached=metricsCache.get(world);if(cached?.version===version)return cached.value;const value=h.snapshots.map(s=>({year:s.year,...s.metrics}));metricsCache.set(world,{version,value});return value}
function eventsThrough(world,year,limit=200){const h=initialize(world),y=clamp(Math.round(Number(year)||0),0,h.currentYear),count=Math.max(1,Math.round(Number(limit)||200));let lo=0,hi=h.events.length;while(lo<hi){const mid=(lo+hi)>>>1;if(h.events[mid].year<=y)lo=mid+1;else hi=mid}return h.events.slice(Math.max(0,lo-count),lo)}
