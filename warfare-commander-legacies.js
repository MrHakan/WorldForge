...
function apply(w){const S=w.warfareSupply;if(!S?.commanders)return S;const C=S.commanders,memorials=(C.legacies||[]).filter(l=>l.memorialEligible).map(l=>memorialFor(S,l));C.memorials=memorials;C.stats=C.stats||{};C.stats.memorials=memorials.length;S.stats=S.stats||{};S.stats.commanderMemorials=memorials.length;if(w.heritage){w.heritage.commanderMemorials=memorials.map(m=>({...m,heritageStatus:m.tier==='legendary'?'national_heritage':m.protected?'protected':'local_register'}));w.heritage.stats=w.heritage.stats||{};w.heritage.stats.commanderMemorials=memorials.length}return S}
